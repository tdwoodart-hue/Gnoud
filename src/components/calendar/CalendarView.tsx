...
export const CalendarView: React.FC = () => {
  ...
  return <div className="mx-auto max-w-2xl">
    ...
    {items.length === 0 ? <EmptyState title="Ngày này đang trống" /> : <div className="space-y-2">{items.map((item) => <div key={item.id} className="flex min-h-[70px] items-center gap-4 rounded-2xl bg-white px-4 ring-1 ring-slate-200"><div className="w-11 text-sm font-bold text-slate-700">{item.time}</div><span className="h-9 w-1 rounded-full" style={{backgroundColor: item.color}} /><div className="min-w-0 flex-1"><p className="truncate font-semibold">{item.title}</p>{item.end && <p className="mt-1 text-xs text-slate-400">đến {item.end}</p>}</div></div>)}</div>}
  </div>;
};
