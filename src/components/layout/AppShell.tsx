...
export const AppShell: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa] font-sans text-slate-950">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-5 border-t border-slate-200 bg-white/95 px-1 pb-[max(10px,env(safe-area-inset-bottom))] pt-1 backdrop-blur md:hidden">
        {PRIMARY_NAV_ITEMS.map((item) => {
          const Icon = icons[item.id];
          const active = activeTab === item.id;
          return (
            <button
              ...
              type="button"
              onClick={() => setActiveTab(item.id)}
              className={`flex min-h-[72px] flex-col items-center justify-center gap-1 rounded-xl text-[10px] font-semibold transition ${
